...
r_t_netrad = np.full([len(lat), len(lon)], np.nan)
...
for xlat in range(len(lat)):
    ...
    for ylon in range(len(lon)):
        ...
        if ~np.isnan(sacksStart_regrid[xlat, ylon]) and ~np.isnan(sacksEnd_regrid[xlat, ylon]):# and crop_ha_regrid[xlat, ylon] > 0:
            ...
            ds_t_growing = ds_tasmax['mx2t'][:,xlat,ylon].sel(time=is_growing_season(ds_tasmax['time.month'], sacks_start_month, sacks_end_month))
            ...
            ds_t_growing_1y = ds_t_growing.resample(time='1Y', loffset='%dM'%(sacks_start_month-1)).mean()
            ds_netrad_growing_1y = ds_netrad_growing.resample(time='1Y', loffset='%dM'%(sacks_start_month-1)).mean()
            
            nn = np.where((~np.isnan(ds_netrad_growing_1y.values)) & (~np.isnan(ds_t_growing_1y.values)))[0]
            
            if nn.size > 10:
                ds_t_growing_1y_detrend = signal.detrend(ds_t_growing_1y.values[nn])
    ...
    #             r_t_et[xlat, ylon] = np.corrcoef(ds_sm_growing_1y.values,ds_evap_growing_1y.values)[0,1]
                r_t_netrad[xlat, ylon] = np.corrcoef(ds_t_growing_1y_detrend,ds_netrad_growing_1y_detrend)[0,1]

        n += 1
                
da_grow_r_t_netrad = xr.DataArray(data   = r_t_netrad, 
                      dims   = ['latitude', 'longitude'],
                      coords = {'latitude':lat, 'longitude':lon},
                      attrs  = {'units'     : 'correlation'
                        })
ds_grow_r_t_netrad = xr.Dataset()
ds_grow_r_t_netrad['r_t_netrad'] = da_grow_r_t_netrad


print('saving netcdf...')
# ds_grow_r_t_et.to_netcdf('r_t_et_era5_crop_restricted_%d_%d.nc'%(years[0], years[1]))
ds_grow_r_t_netrad.to_netcdf('r_t_netrad_era5_%d_%d_detrend-%s.nc'%(years[0], years[1], crop))
