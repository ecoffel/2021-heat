...
regridder_sm_deciles = xe.Regridder(xr.DataArray(data=da_sm_deciles, dims=['percentile', 'lat', 'lon'], coords={'percentile':np.arange(0,101,1), 'lat':sm_era5_last_year.lat, 'lon':sm_era5_last_year.lon}), regridMesh, 'bilinear', reuse_weights=True)
...
ds_grow_sm_on_tmax.to_netcdf('era5_txx_tmean/era5_%s_sm_on_txx_grow_%d.nc'%(crop, year))
ds_grow_et_on_tmax.to_netcdf('era5_txx_tmean/era5_%s_et_on_txx_grow_%d.nc'%(crop, year))